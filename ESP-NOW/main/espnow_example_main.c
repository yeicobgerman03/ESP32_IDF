...
#define I2C_MASTER_SCL_IO    22    /*!< GPIO para el pin SCL */
#define I2C_MASTER_SDA_IO    21    /*!< GPIO para el pin SDA */
#define I2C_MASTER_NUM        I2C_NUM_0  /*!< Puerto I2C número 0 */
#define I2C_MASTER_FREQ_HZ    100000    /*!< Frecuencia de I2C en Hz */
#define I2C_MASTER_TX_BUF_DISABLE  0   /*!< Desactivar buffer de transmisión */
#define I2C_MASTER_RX_BUF_DISABLE  0   /*!< Desactivar buffer de recepción */

#define LCD_ADDR      0x27   /*!< Dirección I2C del LCD */

// Función para enviar un comando o dato al LCD
esp_err_t lcd_write_byte(i2c_port_t i2c_num, uint8_t data) {
    esp_err_t ret;
    uint8_t buffer[1] = { data };

    // Escribir un byte al dispositivo LCD
    ret = i2c_master_write_to_device(i2c_num, LCD_ADDR, buffer, sizeof(buffer), 1000 / portTICK_PERIOD_MS);
    return ret;
}

// Función para inicializar el LCD
void lcd_init(i2c_port_t i2c_num) {
    // Enviar comandos para configurar el LCD
    lcd_write_byte(i2c_num, 0x38);  // Función: 8-bit, 2-line, 5x8 font
    vTaskDelay(50 / portTICK_PERIOD_MS);
    lcd_write_byte(i2c_num, 0x0C);  // LCD ON, sin cursor
    vTaskDelay(50 / portTICK_PERIOD_MS);
    lcd_write_byte(i2c_num, 0x01);  // Limpiar pantalla
    vTaskDelay(2 / portTICK_PERIOD_MS);
    lcd_write_byte(i2c_num, 0x06);  // Modo de incremento de cursor
    vTaskDelay(50 / portTICK_PERIOD_MS);
}

// Función para mostrar un mensaje en el LCD
void lcd_display_message(i2c_port_t i2c_num, const char *message) {
    uint8_t buffer[16];
    snprintf((char *)buffer, sizeof(buffer), "%s", message);
    for (int i = 0; i < 16; i++) {
        lcd_write_byte(i2c_num, buffer[i]);
...
void i2c_master_init() {
    ...
    // Configurar los parámetros de I2C
...
void app_main() {
    ...
    lcd_init(I2C_NUM_0);  // Inicializar LCD

    // Mostrar mensaje en el LCD
    lcd_display_message(I2C_NUM_0, "Hola Mundo");
}
