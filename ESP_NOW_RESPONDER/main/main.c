...
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "freertos/timers.h"  // Agregado para el temporizador
#include "esp_now.h"
#include "esp_mac.h"
#include "esp_wifi.h"
#include "nvs_flash.h"
...
#define LEDC_RESOLUTION LEDC_TIMER_10_BIT
#define LEDC_FREQUENCY 5000
#define TIMEOUT_MS 500  // Tiempo de espera antes de apagar el PWM
...
static state_t current_state = STATE_RECEIVE_DATA;
static uint8_t received_duty_cycle = 0;
static TimerHandle_t timeout_timer;  // Temporizador de inactividad
...
// Función que se ejecuta cuando expira el temporizador
void timeout_callback(TimerHandle_t xTimer)
{
    ESP_LOGW(TAG, "¡No se recibieron datos! Poniendo PWM a 0");
    received_duty_cycle = 0;

    // Actualizar el PWM inmediatamente
    ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, 0);
    ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);

    // Cambiar de estado para volver a recibir datos
    current_state = STATE_UPDATE_PWM;
}
...
void app_main()
{
    ...
    // Crear el temporizador de inactividad (configurado como periódico)
    timeout_timer = xTimerCreate("TimeoutTimer", pdMS_TO_TICKS(TIMEOUT_MS), pdTRUE, NULL, timeout_callback);
